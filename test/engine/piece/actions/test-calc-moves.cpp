...
namespace {
    ...
    piece::army::army_list create_army_list(board::Board board,
                                            std::vector<piece::Piece> white,
                                            std::vector<piece::Piece> black) {
...
TEST(PieceApi_CalcMove, KingMovementObstructed) {
    ...
    auto army_list =
        create_army_list(board, {King(board, "b1"_n)}, {King(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
    // assert
    EXPECT_EQ(moves_all.size(), 1);  // only one piece should move
    EXPECT_EQ(moves_all[0].src, "b1"_n.as_squares(board));
    EXPECT_EQ(moves_all[0].destinations,
              combine_squares(board, "a1"_n, "c1"_n));
}

TEST(PieceApi_CalcMove, KingMovementUnderAttack) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "b1"_n)},
    ...
    EXPECT_EQ(moves_all[0].src, "b1"_n.as_squares(board));
    EXPECT_EQ(moves_all[0].destinations,
              combine_squares(board, "a1"_n, "c1"_n, "a2"_n));
    list_squares(moves_all[0].destinations, board);
}

TEST(PieceApi_CalcMove, KingNoMovementPossible) {
    board::Board board{3, 3};
    auto army_list = create_army_list(
        board, {King(board, "a1"_n)},
...
TEST(PieceApi_CalcMove, KingOnlyMoveIsAttack) {
    ...
    auto army_list =
        create_army_list(board, {King(board, "a1"_n)}, {King(board, "b2"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
    // assert
...
TEST(PieceApi_CalcMove, TwoPieceOnlyRockUnderAttack) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "a1"_n), Rock(board, "b1"_n)},
                         {King(board, "c3"_n), Rock(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
    ...
    EXPECT_EQ(moves_all[1].destinations,
              combine_squares(board, "c1"_n, "b2"_n, "b3"_n));
...
TEST(PieceApi_CalcMove, TwoPieceKingUnderAttackOneAttackerInterceptImpossible) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "b1"_n), Rock(board, "a1"_n)},
                         {King(board, "c3"_n), Rock(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
    ...
    EXPECT_EQ(moves_all[0].src, "b1"_n.as_squares(board));
    EXPECT_EQ(moves_all[0].destinations,
              combine_squares(board, "a2"_n, "c1"_n));
    list_squares(moves_all[0].destinations, board);
}
...
TEST(PieceApi_CalcMove, TwoPieceKingUnderAttackOneAttackerInterceptPossible) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "b1"_n), Rock(board, "a2"_n)},
                         {King(board, "c3"_n), Rock(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
...
TEST(PieceApi_CalcMove, TwoPieceKingUnderAttackOneAttackerInterceptPossible2) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "b1"_n), Rock(board, "a3"_n)},
                         {King(board, "c3"_n), Rock(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
...
TEST(PieceApi_CalcMove, TwoPieceMovementWouldEndangerKing) {
    board::Board board{3, 3};
    auto army_list =
        create_army_list(board, {King(board, "b1"_n), Rock(board, "b2"_n)},
                         {King(board, "c3"_n), Rock(board, "b3"_n)});
    // act
    auto moves_all =
        piece::api::calc_possible_moves(army_list[0], board, army_list);
...
TEST(PieceApi_CalcMove,
     TwoPieceKingUnderAttackOneAttackerInterceptWouldEndangerKing) {
    board::Board board{3, 3};
    auto army_list = create_army_list(
        board, {King(board, "c1"_n), Rock(board, "c2"_n)},
