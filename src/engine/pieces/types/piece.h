...
namespace piece {
    ...
    class Piece final {
        public:
            using update_fn = void (*)(piece::Piece& piece,
        ...
        public:
            ...
            board::bitmap::Squares observed{
                0};  // = attackable & ~positions_of_friendly_pieces
            board::bitmap::Squares attackable{
                0};  // all squares the piece can move to (not considering
                     // checkmate)

            void update_observed_and_attackable(const board::Board& board,
                                                sqrs pos_all,
                                                sqrs pos_hostile_armies);
            void mark_as_dead();
            bool is_alive() const;
