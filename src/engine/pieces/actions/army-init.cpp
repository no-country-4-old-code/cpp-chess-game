...
namespace {
    ...
    struct Positions {
            ...
            std::array<board::bitmap::Squares, piece::army::max_num_of_armies>
...
namespace {
    ...
    void update_pieces(const Positions &positions, const board::Board &board,
                       piece::army::army_list &army_list) {
        size_t idx_army = 0;
        for (auto &army : army_list) {
            auto positions_hostile_pieces =
                positions.positions_all & ~positions.army_positions_lookup.at(idx_army);
            ++idx_army;
