...
8
    */
    ...
    auto army_list = create_army_list(
        b,
        {Queen(b, "d1"_n), Pawn(b, "f2"_n, down), Pawn(b, "g2"_n, down), Knight(b, "b4"_n),
         Pawn(b, "e4"_n, down), Rock(b, "f4"_n), King(b, "b5"_n), Pawn(b, "c5"_n, down)},
        {
            Pawn(b, "g4"_n, up),
            King(b, "e5"_n),
            Rock(b, "g5"_n),
...
8
    */
    auto army_list = create_army_list(
        b, {Queen(b, "c5"_n), Pawn(b, "h2"_n, down), King(b, "f3"_n), Rock(b, "f7"_n)},
        {Knight(b, "h5"_n), King(b, "h1"_n)});
    auto ai = ChessAI(army_list, b);
    // display::display_board(b, army_list);
    ai.make_move(army_list[0]);
    ai.make_move(army_list[1]);
...
8
    ...
    */
    auto army_list =
        create_army_list(b, {King(b, "e1"_n), Queen(b, "h1"_n), Rock(b, "e4"_n), Rock(b, "c4"_n)},
                         {Knight(b, "g4"_n), King(b, "d5"_n)});
    // display::display_board(b, army_list);
    auto ai = ChessAI(army_list, b);
    ai.make_move(army_list[0]);
    ai.make_move(army_list[1]);
