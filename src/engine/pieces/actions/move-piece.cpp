...
namespace piece::api {
    void move_piece(const board::bitmap::Squares src, const board::bitmap::Squares dest,
                    const board::Board &board, piece::army::army_list &army_list) {
        ...
        for (auto &army : army_list) {
            auto positions_hostile_pieces =
                positions_all & ~army_positions_lookup[idx_army];  // NOLINT
            ++idx_army;
            for (auto &current : army.pieces) {
                if (((current.observed | current.position) & affected_squares)) {
                    current.update(board, positions_all, positions_hostile_pieces);
                }
            }
        }
    }
